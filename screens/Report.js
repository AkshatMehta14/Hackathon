...
export default class HomeScreen extends Component {
    ...
    grouping = async (description) => {
        ...
        try {
            ...
            if (!res.ok) {
                console.error("Error in grouping function:", res.status, res.statusText);
                return;
            }

            const json = await res.json();
            if (!json.choices || !json.choices[0] || !json.choices[0].text) {
                console.error("Error in grouping function: Unexpected API response format.");
                return;
            }
